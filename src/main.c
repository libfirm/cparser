...
int action_compile(const char *argv0)
{
	...
	// depending on the compilation mode, the lto backend
	// can be more restrictive when merging entities
	lto_set_mode_is_compile_executable(mode == MODE_COMPILE_ASSEMBLE_LINK);
